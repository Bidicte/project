...
interface DeleteTarifConfirmationModalProps {
  isOpen: boolean
  id: string
  type: string
  onClose: () => void
...
export function DeleteTarifConfirmationModal({
  isOpen,
  id,
  type,
  onClose,
  onConfirm,
}: DeleteTarifConfirmationModalProps) {
    ...
    <div className="fixed inset-0 bg-gray-300 bg-opacity-80 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-md w-[350px] p-6 text-center" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="text-sm text-gray-600 mb-6">
          Voulez-vous supprimer le tarif <strong> {type} </strong> ?<br />Retenez que toute action de suppression est irréversible.
        </p>
